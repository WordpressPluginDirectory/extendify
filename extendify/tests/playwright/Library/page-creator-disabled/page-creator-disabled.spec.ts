...
test('Library modal auto-opens on the page editor when AI Page Creator is disabled', async ({
	admin,
	page,
}) => {
	await admin.visitAdminPage('post-new.php', 'post_type=page');

	const modal = page.locator('.extendify-library-modal');
	await expect(modal).toBeVisible({ timeout: 15_000 });
	await expect(page.getByTestId('category-control')).toBeVisible();
});
